...
#################################padding_price=None###########################################################################
        ...
        decode_out = torch.squeeze(decode_out,2)      #[109,11,1,2*12]->[109,11,2*12]
        previous_w = previous_w.permute(0,2,1)        #[128,1,11]->[128,11,1]
        out = torch.cat([decode_out,previous_w],2)    #[109,11,2*12]  cat [128,11,1] -> [128,11,2*12+1]
###################################  Decision making ##################################################
        ...
        out = torch.cat([bias,out],1)                 #[128,11,1] cat [128,1,1] -> [109,12,1]
        out2 = torch.cat([bias2,out2],1)              #[128,11,1] cat [128,1,1] -> [109,12,1]
...
def train_one_step(DM,x_window_size,model,loss_compute,local_context_length):
    ...
    batch_last_w = batch["last_w"]  #(109, 11)
    batch_w = batch["setw"]     
#############################################################################
    ...
    price_series_mask = (torch.ones(src.size()[1],1,x_window_size)==1)   #[109, 1, 31] 
    ...
    out = model.forward(src, currt_price, previous_w,  #[109,1,11]   [109, 11, 31, 3]) torch.Size([109, 11, 3]
...
def test_online(DM,x_window_size,model,evaluate_loss_compute,local_context_length):
    ...
    tst_batch_input = tst_batch["X"]          #(1,4,11,2807)
    tst_batch_y = tst_batch["y"]              #(128, 4, 11)
    tst_batch_last_w = tst_batch["last_w"]
#        print("batch_last_w",np.shape(batch_last_w),"\r\n")   #(128, 11)
...
#        print("src_mask:",src_mask.size())   
    ...
    long_term_tst_currt_price=long_term_tst_currt_price[:,:,x_window_size-1:,:]   #(11,109,31+9,3)->(11,109,10,3)
###############################################################################################    
#        trg_mask = make_std_mask(previous_w,src.size()[1])   #pad=0   [109, 1, 1] 对 
#########################################################################################
    tst_trg_mask = make_std_mask(long_term_tst_currt_price[:,:,0:1,:],long_term_tst_src.size()[1])
#        print("trg_mask:",trg_mask.size())      

    tst_batch_y=tst_batch_y.transpose((0,3,2,1))   #(128, 4, 11, 10) ->(128,10,11,4)
...
#            print("train_net src",src.size(),"j:j+x_window_size",j,j+x_window_size,x_window_size)
        ...
        if(j==0):
                #long_term_w.append(out)  #[10,109,1,12] 
...
#                    torch.save(model, model_dir+'/'+'transformer_random_sample_pre_price_11_4_conv_multihead_asset_relation_only_encoder_pre_w.pkl')
